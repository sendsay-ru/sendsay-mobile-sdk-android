...
import com.sendsay.sdk.testutil.close
...
internal class FlushManagerTest : SendsaySDKTest() {
    private lateinit var manager: FlushManager
    private lateinit var repo: EventRepository
    private lateinit var connectionManager: ConnectionManager
    private lateinit var service: SendsayService
    ...
    private fun setup(connected: Boolean, serviceSuccess: Boolean) {
        ...
        service = spyk(SendsayMockService(serviceSuccess))
    ...
    private fun createTestEvent(includeProject: Boolean, type: String? = "test_event"): ExportedEvent {
        val event = ExportedEvent(
                ...
                sendsayProject = if (includeProject)
                    SendsayProject("mock_base_url.com", "mock_project_token", "mock_auth")
                else null
        )
        repo.add(event)
        return event
    ...
    fun `should flush event`() {
        ...
        verify {
            service.postEvent(SendsayProject("mock_base_url.com", "mock_project_token", "mock_auth"), any())
        }
    }

    @Test
    fun `should flush old event without sendsay project`() {
        setup(connected = true, serviceSuccess = true)
        ...
        verify {
            service.postEvent(SendsayProject("https://api.sendsay.com", "old-project-id", null), any())
        }
    }
    ...
    fun `should only flush once`() {
        ...
        waitForIt {
            thread(start = true) {
                ...
                manager.flushData {
                    // SendsayMockService returns false but test accepts FlushFinishedCallback to be called
