...
internal class FcmManagerImplTest {
    ...
    fun `should generate correct intents for 2 notifications`() {
        ...
        assertEquals(
            ...
            intent1.extras?.get(SendsayExtras.EXTRA_ACTION_INFO)
        )

        assertEquals(
            NotificationAction("notification", null, "app://mock-url-2"),
            intent2.extras?.get(SendsayExtras.EXTRA_ACTION_INFO)
        )
    }
    ...
    fun `should track token in ON_TOKEN_CHANGE mode`() {
        manager.trackToken("token", SendsayConfiguration.TokenFrequency.ON_TOKEN_CHANGE, TokenType.FCM)
        manager.trackToken("token", SendsayConfiguration.TokenFrequency.ON_TOKEN_CHANGE, TokenType.FCM)
        manager.trackToken("other_token", SendsayConfiguration.TokenFrequency.ON_TOKEN_CHANGE, TokenType.FCM)
    ...
    fun `should track token in DAILY mode`() {
        ...
        manager.trackToken("token", SendsayConfiguration.TokenFrequency.DAILY, TokenType.FCM)
        verify(exactly = 1) { pushTokenRepository.setTrackedToken("token", any(), TokenType.FCM, true) }
        manager.trackToken("other_token", SendsayConfiguration.TokenFrequency.DAILY, TokenType.FCM)
    ...
    fun `should track token in EVERY_LAUNCH mode`() {
        manager.trackToken("token", SendsayConfiguration.TokenFrequency.EVERY_LAUNCH, TokenType.FCM)
        manager.trackToken("token", SendsayConfiguration.TokenFrequency.EVERY_LAUNCH, TokenType.FCM)
        manager.trackToken("other_token", SendsayConfiguration.TokenFrequency.EVERY_LAUNCH, TokenType.FCM)
    ...
    fun `should track empty token in ON_TOKEN_CHANGE if permission denied but required (forcibly)`() {
        ...
        manager.trackToken(pushToken, SendsayConfiguration.TokenFrequency.ON_TOKEN_CHANGE, TokenType.FCM)
    ...
    fun `should track empty token in DAILY if permission denied but required (forcibly)`() {
        ...
        every { NotificationsPermissionReceiver.isPermissionGranted(any()) } returns permissionGranted
        // track same token again
        manager.trackToken(pushToken, SendsayConfiguration.TokenFrequency.DAILY, TokenType.FCM)
        // has to store exactly-same token again
        verify(exactly = 1) { pushTokenRepository.setTrackedToken(pushToken, any(), TokenType.FCM, permissionGranted) }
        verify(exactly = 1) { eventManager.track(
            Constants.EventTypes.push,
    ...
    fun `should track empty token in EVERY_LAUNCH if permission denied but required (forcibly)`() {
        ...
        every { NotificationsPermissionReceiver.isPermissionGranted(any()) } returns permissionGranted
        // track same token again
        manager.trackToken(pushToken, SendsayConfiguration.TokenFrequency.EVERY_LAUNCH, TokenType.FCM)
        // has to store exactly-same token again
        verify(exactly = 1) { pushTokenRepository.setTrackedToken(pushToken, any(), TokenType.FCM, permissionGranted) }
        verify(exactly = 1) { eventManager.track(
            Constants.EventTypes.push,
    ...
    fun `should track new full token in ON_TOKEN_CHANGE if permission denied but not required`() {
        ...
        every { NotificationsPermissionReceiver.isPermissionGranted(any()) } returns permissionGranted
        // track same token again
        manager.trackToken(pushToken2, SendsayConfiguration.TokenFrequency.ON_TOKEN_CHANGE, TokenType.FCM)
    ...
    fun `should use push icon from configuration`() {
        manager = FcmManagerImpl(
            context,
            SendsayConfiguration(pushIcon = 123),
            eventManager,
            pushTokenRepository,
            trackingConsentManager
        )
