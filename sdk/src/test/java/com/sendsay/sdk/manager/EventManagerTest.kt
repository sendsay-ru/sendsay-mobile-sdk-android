...
internal class EventManagerTest : SendsaySDKTest() {
    ...
    lateinit var projectFactory: SendsayProjectFactory

    lateinit var addedEvents: ArrayList<ExportedEvent>

    fun setup(context: Context, configuration: SendsayConfiguration, flushMode: FlushMode) {
        mockkObject(Sendsay)
        every { Sendsay.flushMode } returns flushMode
        ...
        projectFactory = SendsayProjectFactory(context, configuration)
    ...
    fun `should track event`() {
        ...
        setup(
            ...
            SendsayConfiguration(projectToken = "mock-project-token"),
            FlushMode.MANUAL
        )
    ...
    fun `should track event for all projects`() {
        ...
        setup(
            ...
            SendsayConfiguration(
                ...
                projectRouteMap = hashMapOf(EventType.INSTALL to arrayListOf(
                    SendsayProject("mock_base_url1.com", "token1", "mock_auth"),
                    SendsayProject("mock_base_url2.com", "token2", "mock_auth")
                ))
            ),
            FlushMode.MANUAL
        )
        ...
        assertEquals(
            SendsayProject("https://api.sendsay.com", "mock-project-token", null),
            addedEvents[0].sendsayProject
        )
        assertEquals(
            SendsayProject("mock_base_url1.com", "token1", "mock_auth"),
            addedEvents[1].sendsayProject
        )
        assertEquals(
            SendsayProject("mock_base_url2.com", "token2", "mock_auth"),
            addedEvents[2].sendsayProject
        )
    }
