...
internal class FetchManagerTest : SendsaySDKTest() {
    private lateinit var server: MockWebServer

    @Before
    fun before() {
        server = SendsayMockServer.createServer()
    }
    ...
    fun `should call onSuccess when server returns valid non-empty data for consents`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(true, getResponse(consentsResponse)),
    ...
    fun `should call onFailure when server returns invalid json for consents`() {
        waitForIt {
            FetchManagerImpl(SendsayMockService(true, getResponse("{{{{")), SendsayGson.instance).fetchConsents(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
    ...
    fun `should call onFailure when server returns false state for consents`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(true, getResponse("{success: false, results:[]}")),
                SendsayGson.instance
            ).fetchConsents(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                onSuccess = { _ -> it.fail("This should not happen") },
    ...
    fun `should call onFailure when server returns empty data json for consents`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(true, getResponse("{success: true, results:[]}")),
                SendsayGson.instance
            ).fetchConsents(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                onSuccess = { _ -> it.fail("This should not happen") },
    ...
    fun `should call onFailure when server returns null for consents`() {
        waitForIt {
            FetchManagerImpl(SendsayMockService(true, getResponse(null)), SendsayGson.instance).fetchConsents(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                onSuccess = { _ -> it.fail("This should not happen") },
                onFailure = { _ -> it() }
            )
    ...
    fun `should call onFailure when server returns error code for consents`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(false, getResponse(consentsResponse)),
                SendsayGson.instance
            ).fetchConsents(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                onSuccess = { _ -> it.fail("This should not happen") },
    ...
    fun `should call onSuccess when server returns null data json InApps`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(true, getResponse("{success:true}")),
                SendsayGson.instance
            ).fetchInAppMessages(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")),
    ...
    fun `should call onFailure when server returns false state InApps`() {
        waitForIt {
            val emptyResponseInstance = SendsayMockService(true, getResponse("{success:false}"))
            val fetchManagerImpl = FetchManagerImpl(emptyResponseInstance, SendsayGson.instance)
    ...
    fun `should call onFailure when server returns raw-empty InApps`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(true, getResponse("")),
                SendsayGson.instance
            ).fetchInAppMessages(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")),
    ...
    fun `should call onFailure when server returns null InApps`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(true, getResponse(null)),
                SendsayGson.instance
            ).fetchInAppMessages(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")),
    ...
    fun `should call onFailure when server returns error InApps`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(false, getResponse("{}")),
                SendsayGson.instance
            ).fetchInAppMessages(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")),
    ...
    fun `should call onSuccess when server returns null data AppInbox`() {
        waitForIt {
            val emptyResponseInstance = SendsayMockService(true, getResponse(
    ...
    fun `should call onFailure when server returns null AppInbox`() {
        waitForIt {
            val emptyResponseInstance = SendsayMockService(true, getResponse(null))
            val fetchManagerImpl = FetchManagerImpl(emptyResponseInstance, SendsayGson.instance)
            fetchManagerImpl.fetchAppInbox(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")),
    ...
    fun `should call onFailure when server returns error AppInbox`() {
        waitForIt {
            val emptyResponseInstance = SendsayMockService(false, getResponse("{}"))
            val fetchManagerImpl = FetchManagerImpl(emptyResponseInstance, SendsayGson.instance)
            fetchManagerImpl.fetchAppInbox(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")),
    ...
    fun `should call onSuccess when server returns valid non-empty data for recommendations`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(true, getResponse(recommendationsResponse)),
                SendsayGson.instance
            ).fetchRecommendation(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
    ...
    fun `should call onFailure when server returns error code for recommendations`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(false, getResponse(recommendationsResponse)),
                SendsayGson.instance
            ).fetchRecommendation(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerRecommendationRequest(
    ...
    fun `should call onFailure when server returns failure state MarkAsRead AppInbox action`() {
        waitForIt {
            FetchManagerImpl(
                SendsayMockService(true, getResponse("{success: false}")),
                SendsayGson.instance
            ).markAppInboxAsRead(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")),
    ...
    fun `should call onSuccess when server returns some data for Segments`() {
        ...
        waitForIt {
            val emptyResponseInstance = SendsayMockService(true, getResponse(SegmentTest.segmentationsJson))
            val fetchManagerImpl = FetchManagerImpl(emptyResponseInstance, SendsayGson.instance)
            fetchManagerImpl.fetchSegments(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")).apply {
    ...
    fun `should call onFailure when server returns false state for Segments`() {
        waitForIt {
            val emptyResponseInstance = SendsayMockService(false, getResponse(null))
            val fetchManagerImpl = FetchManagerImpl(emptyResponseInstance, SendsayGson.instance)
            fetchManagerImpl.fetchSegments(
                SendsayProject("mock-base-url.com", "mock-project-token", "mock-auth"),
                CustomerIds(hashMapOf("user" to "test")).apply {
