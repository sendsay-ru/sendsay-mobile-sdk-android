...
internal class CampaignClickEventTests : SendsaySDKTest() {
    ...
    fun prepareForTest() {
        ...
        eventRepository = Sendsay.componentForTesting.eventRepository
        campaignRepository = Sendsay.componentForTesting.campaignRepository
    ...
    fun testHandleIntent_normal() = runInSingleThread { idleThreads ->
        ...
        assertTrue(Sendsay.handleCampaignIntent(universalLinkIntent, context))
        idleThreads()
    ...
    fun testHandleIntent_invalid_NullData() {
        ...
        val handled = Sendsay.handleCampaignIntent(universalLinkIntent, context)
        val storedEvents = eventRepository.all()
    ...
    fun testHandleIntent_invalid_InvalidUrl_DL() {
        ...
        val handled = Sendsay.handleCampaignIntent(deepLinkIntent, context)
        val storedEvents = eventRepository.all()
    ...
    fun testHandleIntent_checkEventProperties() = runInSingleThread { idleThreads ->
        val universalLinkIntent = createUniversalLinkIntent()
        Sendsay.handleCampaignIntent(universalLinkIntent, context)
        idleThreads()
    ...
    fun testHandleIntent_oldCampaign() {
        ...
        runBlocking {
            delay(Sendsay.campaignTTL.plus(1).times(1000).toLong())
        }
    ...
    fun testHandleIntent_sessionUpdate_valid_UL() = runInSingleThread { idleThreads ->
        ...
        Sendsay.handleCampaignIntent(universalLinkIntent, context)
        val campaignEvent = campaignRepository.get()!!
        Sendsay.trackSessionStart(currentTimeSeconds())
        idleThreads()
        val storedEvents = eventRepository.all()
    ...
    fun testHandleIntent_sessionUpdate_valid_DL() = runInSingleThread { idleThreads ->
        val deepLinkIntent = createDeepLinkIntent()
        Sendsay.handleCampaignIntent(deepLinkIntent, context)
        val campaignEvent = campaignRepository.get()!!
        Sendsay.trackSessionStart(currentTimeSeconds())
        idleThreads()
        val storedEvents = eventRepository.all()
    ...
    private fun flushDataSync() {
        waitForIt { Sendsay.flushData { _ -> it() } }
    }
