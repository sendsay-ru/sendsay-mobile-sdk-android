...
import com.sendsay.sdk.manager.AppInboxAdapter
import com.sendsay.sdk.models.AppInboxMessateType.HTML
import com.sendsay.sdk.models.AppInboxMessateType.PUSH
...
import com.sendsay.sdk.models.MessageItemAction.Type
import com.sendsay.sdk.models.MessageItemAction.Type.APP
import com.sendsay.sdk.models.MessageItemAction.Type.BROWSER
import com.sendsay.sdk.models.MessageItemAction.Type.DEEPLINK
import com.sendsay.sdk.models.MessageItemAction.Type.NO_ACTION
import com.sendsay.sdk.models.MessageItemContent
import com.sendsay.sdk.util.HtmlNormalizer
import com.sendsay.sdk.util.HtmlNormalizer.ActionInfo
...
import com.sendsay.sdk.util.URLUtils
import com.sendsay.sdk.util.logOnException
import com.sendsay.sdk.view.AppInboxDetailFragment
import com.sendsay.sdk.view.AppInboxDetailView
import com.sendsay.sdk.view.AppInboxListActivity
import com.sendsay.sdk.view.AppInboxListFragment
import com.sendsay.sdk.view.AppInboxListView
...
open class DefaultAppInboxProvider : AppInboxProvider {
    ...
    override fun getAppInboxButton(context: Context): Button {
        ...
        Sendsay.deintegration.registerForIntegrationStopped(object : OnIntegrationStoppedCallback {
    ...
    override fun getAppInboxListView(context: Context, onItemClicked: (MessageItem, Int) -> Unit): View {
        ...
        val onItemClickedProxy = { message: MessageItem, position: Int ->
            ...
            Sendsay.markAppInboxAsRead(message, null)
            runCatching {
...
//            PUSH -> showPushMessageDetail(source, target)
    ...
    private fun showHtmlMessageDetail(source: MessageItem, target: AppInboxDetailView) {
        ...
        val bitmapCache = Sendsay.getComponent()?.drawableCache
        val fontCache = Sendsay.getComponent()?.fontCache
        if (bitmapCache == null || fontCache == null) {
            throw Exception("Sendsay SDK was not initialized properly!")
        }
...
//    }
        ...
        source?.action?.let { mainAction ->
            if (SUPPORTED_MESSAGE_ACTION_TYPES.contains(mainAction.type)) {
                target.add(MessageItemAction().apply {
                    ...
                    url = mainAction.url
                    title = context.getString(R.string.sendsay_inbox_mainActionTitle)
                })
            }
        }
        source?.actions
    ...
    private fun invokeAction(action: MessageItemAction, message: MessageItem, context: Context) {
        ...
        Sendsay.trackAppInboxClick(action, message)
        when (action.type) {
            APP -> {
