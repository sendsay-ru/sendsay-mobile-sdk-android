...
import ru.sendsay.sdk.models.InAppMessageButtonType.BROWSER
import ru.sendsay.sdk.models.InAppMessageButtonType.DEEPLINK
...
import ru.sendsay.sdk.style.InAppRichstylePayloadBuilder
...
import ru.sendsay.sdk.util.GdprTracking
import ru.sendsay.sdk.util.ensureOnBackgroundThread
import ru.sendsay.sdk.util.ensureOnMainThread
...
internal class InAppMessageManagerImpl(
    ...
    internal fun reload(
        ...
        if (!Sendsay.isInAppMessagesEnabled) {
            Logger.e(this, "In-app fetch failed, Sendsay.isInAppMessagesEnabled = false")
            callback?.invoke(Result.failure(Exception("In-app fetch failed, Sendsay.isInAppMessagesEnabled = false")))
            return
        }
        ...
        if (!SendsayContextProvider.applicationIsForeground) {
            Logger.d(
                this,
        ...
        fetchManager.fetchInAppMessages(
            sendsayProject = projectFactory.mainSendsayProject,
...
//                    drawableCache.clearExcept(loadImageUrls(result.results))
        ...
        runOnBackgroundThread(message.delay ?: 0) {
            ...
            val presented = presenter.show(
                ...
                actionCallback = { activity, button ->
                    ...
                    if (Sendsay.inAppMessageActionCallback.trackActions) {
                    ...
                    ensureOnMainThread {
                        Sendsay.inAppMessageActionCallback.inAppMessageClickAction(
                            message,
                            buttonInfo,
                            activity
                        )
                    }
                    if (!Sendsay.inAppMessageActionCallback.overrideDefaultBehavior) {
                        ensureOnMainThread {
                ...
                dismissedCallback = { activity, userInteraction, cancelButton ->
                    ...
                    ensureOnMainThread {
                        Sendsay.inAppMessageActionCallback.inAppMessageCloseAction(
                            message,
                            buttonInfo,
                            userInteraction,
                            activity
    ...
    private fun trackShowEvent(message: InAppMessage) {
        ...
        Sendsay.telemetry?.reportEvent(
            ru.sendsay.sdk.telemetry.model.EventType.SHOW_IN_APP_MESSAGE,
