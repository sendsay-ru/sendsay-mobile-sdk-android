...
internal open class FcmManagerImpl(
        ...
        val shouldUpdateToken = run {
            ...
            if (lastTrackDateInMilliseconds == null) {
                ...
                when (tokenTrackFrequency ?: configuration.tokenTrackFrequency) {
                    SendsayConfiguration.TokenFrequency.ON_TOKEN_CHANGE ->
                        token != pushTokenRepository.get() ||
                            permissionGranted != pushTokenRepository.getLastPermissionFlag()
                    SendsayConfiguration.TokenFrequency.EVERY_LAUNCH -> true
                    SendsayConfiguration.TokenFrequency.DAILY -> !DateUtils.isToday(lastTrackDateInMilliseconds)
                }
            }
        }
    ...
    override fun handleRemoteMessage(
        ...
        if (payload.notificationAction.action == SendsayNotificationActionType.SELFCHECK) {
    ...
    protected open fun trackDeliveredPush(
        ...
        Sendsay.notifyCallbacksForNotificationDelivery(payload)
    }
    ...
    private fun getPushReceiverIntent(payload: NotificationPayload): Intent {
        return SendsayPushTrackingActivity.getClickIntent(
            application,
    ...
    private fun generateActionPendingIntent(
        ...
        trackingIntent.putExtra(SendsayExtras.EXTRA_ACTION_INFO, actionInfo)

        return when (action) {
            SendsayNotificationActionType.BROWSER -> {
                trackingIntent.action = SendsayExtras.ACTION_URL_CLICKED
                getUrlIntent(requestCode, trackingIntent, actionInfo.url)
            }
            SendsayNotificationActionType.DEEPLINK -> {
                trackingIntent.action = SendsayExtras.ACTION_DEEPLINK_CLICKED
                getUrlIntent(requestCode, trackingIntent, actionInfo.url)
            }
            // NotificationPayload.Actions.APP
            else -> {
                trackingIntent.action = SendsayExtras.ACTION_CLICKED
