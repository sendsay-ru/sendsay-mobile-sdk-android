...
internal class InAppMessagePresenter(
    ...
    private fun chooseViewType(
        messageType: InAppMessageType,
        richStyled: Boolean
    ): InAppMessageViewType {
    ...
    fun show(
        ...
        val presenterDismissedCallback =
            { userInteraction: Boolean, cancelButton: InAppMessagePayloadButton? ->
                Logger.i(
                    this,
                    "InApp dismissed by user: ${if (userInteraction) "true" else "false"}"
                )
                presentedMessage = null
