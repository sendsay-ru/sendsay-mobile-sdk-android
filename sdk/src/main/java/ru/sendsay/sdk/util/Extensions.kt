...
fun <T> Result<T>.returnOnException(mapThrowable: (e: Throwable) -> T): T {
    return this.getOrElse {
        try {
            Logger.e(Sendsay, "Sendsay Safe Mode wrapper caught unhandled error", it)
        } catch (e: Throwable) {
...
fun Result<Unit>.logOnException() {
    ...
    if (exception != null) {
        try {
            Logger.e(Sendsay, "Sendsay Safe Mode wrapper caught unhandled error", exception)
        } catch (e: Throwable) {
            // cannot log problem, swallowing
        }
        if (!Sendsay.safeModeEnabled) {
...
fun Context.isReactNativeSDK(): Boolean {
    return isOtherSDK("SendsayReactNativeSDK")
}

fun Context.isCapacitorSDK(): Boolean {
    return isOtherSDK("SendsayCapacitorSDK")
}

fun Context.isFlutterSDK(): Boolean {
    return isOtherSDK("SendsayFlutterSDK")
}

fun Context.isXamarinSDK(): Boolean {
    return isOtherSDK("SendsayXamarinSDK")
}
...
fun Context.getReactNativeSDKVersion(): String? {
    return getSDKVersion("SendsayReactNativeSDKVersion")
}

fun Context.getCapacitorSDKVersion(): String? {
    return getSDKVersion("SendsayCapacitorSDKVersion")
}

fun Context.getFlutterSDKVersion(): String? {
    return getSDKVersion("SendsayFlutterSDKVersion")
}

fun Context.getXamarinSDKVersion(): String? {
    return getSDKVersion("SendsayXamarinSDKVersion")
}
...
fun Context.isCalledFromExampleApp(): Boolean = runCatching {
    ...
    return appInfo.metaData.getBoolean("SendsayExampleApp", false)
}.returnOnException { false }
...
internal fun InAppContentBlock.deepCopy(): InAppContentBlock {
    val sourceJson = SendsayGson.instance.toJson(this)
    val target: InAppContentBlock = SendsayGson.instance.fromJson(sourceJson)
