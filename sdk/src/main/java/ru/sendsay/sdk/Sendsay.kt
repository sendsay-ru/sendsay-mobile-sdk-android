...
import ru.sendsay.sdk.manager.ConfigurationFileManager
import ru.sendsay.sdk.manager.FcmManager
import ru.sendsay.sdk.manager.TimeLimitedFcmManagerImpl
...
import ru.sendsay.sdk.manager.TrackingConsentManager.MODE.IGNORE_CONSENT
...
import ru.sendsay.sdk.models.SendsayConfiguration.TokenFrequency
import ru.sendsay.sdk.models.SendsayNotificationActionType
import ru.sendsay.sdk.models.SendsayProject
import ru.sendsay.sdk.models.FetchError
import ru.sendsay.sdk.models.FlushMode
import ru.sendsay.sdk.models.FlushMode.APP_CLOSE
import ru.sendsay.sdk.models.FlushMode.IMMEDIATE
import ru.sendsay.sdk.models.FlushMode.MANUAL
import ru.sendsay.sdk.models.FlushMode.PERIOD
...
import ru.sendsay.sdk.models.PushNotificationDelegate
...
import ru.sendsay.sdk.services.AppInboxProvider
import ru.sendsay.sdk.services.SendsayContextProvider
import ru.sendsay.sdk.services.SendsayInitManager
import ru.sendsay.sdk.services.MessagingUtils
import ru.sendsay.sdk.services.SendsayDeintegrateManager
import ru.sendsay.sdk.services.inappcontentblock.ContentBlockCarouselViewController.Companion.DEFAULT_MAX_MESSAGES_COUNT
import ru.sendsay.sdk.services.inappcontentblock.ContentBlockCarouselViewController.Companion.DEFAULT_SCROLL_DELAY
...
import ru.sendsay.sdk.util.VersionChecker
import ru.sendsay.sdk.util.addAppStateCallbacks
import ru.sendsay.sdk.util.currentTimeSeconds
import ru.sendsay.sdk.util.ensureOnBackgroundThread
import ru.sendsay.sdk.util.handleClickedPushUpdate
import ru.sendsay.sdk.util.handleReceivedPushUpdate
import ru.sendsay.sdk.util.isViewUrlIntent
import ru.sendsay.sdk.util.logOnException
import ru.sendsay.sdk.util.logOnExceptionWithResult
import ru.sendsay.sdk.util.returnOnException
import ru.sendsay.sdk.util.runOnMainThread
import ru.sendsay.sdk.util.toMap
import ru.sendsay.sdk.view.ContentBlockCarouselView
import ru.sendsay.sdk.view.InAppContentBlockPlaceholderView
import ru.sendsay.sdk.view.InAppMessagePresenter
import ru.sendsay.sdk.view.InAppMessageView
...
object Sendsay {
     ...
     */
    val tokenTrackFrequency: SendsayConfiguration.TokenFrequency
        get() = runCatching {
     ...
     * "sendsay_configuration.json" that must be inside the "assets" folder of your application
     */
    ...
    /**
     * This is the main init method that should be called to initialize Sendsay SDK
     */
    @Synchronized
...
//        isAppInboxEnabled = configuration.isAppInboxEnabled
        ...
        SendsayContextProvider.registerForegroundStateListener(object : OnForegroundStateListener {
    ...
    /**
     * Manually push all events to Sendsay
     */
    ...
    fun getConsents(
        ...
        initGate.waitForInitialize {
            ...
            telemetry?.reportEvent(ru.sendsay.sdk.telemetry.model.EventType.FETCH_CONSENTS)
        }
    }.logOnException()
    ...
    fun fetchRecommendation(
        ...
        initGate.waitForInitialize {
            ...
            telemetry?.reportEvent(ru.sendsay.sdk.telemetry.model.EventType.FETCH_RECOMMENDATION)
        }
    }.logOnException()
    ...
    /**
     * Manually track Hms Token to Sendsay API.
     */
    ...
    /**
     * Manually track delivered push notification to Sendsay API.
    ...
    /**
     * Manually track clicked push notification to Sendsay API.
    ...
    fun isSendsayPushNotification(messageData: Map<String, String>?): Boolean {
     ...
     *
     * @return true if notification is coming from Sendsay servers, false otherwise.
     */
    ...
    private fun getTrackingConsentManager(): TrackingConsentManager? {
        ...
        val applicationContext = SendsayContextProvider.applicationContext
    ...
    internal fun getPushNotificationRepository(): PushNotificationRepository? {
        ...
        try {
            return PushNotificationRepositoryImpl(SendsayPreferencesImpl(applicationContext))
        } catch (e: Exception) {
            Logger.e(
                this,
    ...
    private fun initializeSdk(context: Context) {
        this.component = SendsayComponent(this.configuration, context)
        ...
        ensureOnBackgroundThread {
            telemetry?.reportEvent(
                ru.sendsay.sdk.telemetry.model.EventType.EVENT_COUNT,
     ...
     */
    fun anonymize(
        sendsayProject: SendsayProject? = null,
        ...
        requireInitialized(
            ...
            initializedBlock = {
                ...
                telemetry?.reportEvent(ru.sendsay.sdk.telemetry.model.EventType.ANONYMIZE)
            }
        )
    }.logOnException()
...
// View is not kept as field but generating from scratch to avoid memory leaks
    ...
    fun fetchAppInbox(callback: ((List<MessageItem>?) -> Unit)) = runCatching {
        initGate.waitForInitialize {
            ...
            telemetry?.reportEvent(ru.sendsay.sdk.telemetry.model.EventType.TRACK_INBOX_FETCH)
        }
    }.logOnException()
    ...
    internal fun getComponent(): SendsayComponent? = runCatching<SendsayComponent?> {
        requireInitialized<SendsayComponent>(
            initializedBlock = {
                component
            }
        )
    ...
    fun clearLocalCustomerData() = runCatching {
        ...
        if (SendsayConfigRepository.get(context) == null) {
    ...
    internal fun processPushNotificationClickInternally(openedPushDataIntent: Intent) {
        ...
        SendsayContextProvider.applicationContext?.let { context: Context ->
            Intent().also { broadcastIntent ->
                broadcastIntent.action = openedPushDataIntent.action
                broadcastIntent.putExtra(SendsayExtras.EXTRA_ACTION_INFO, action)
    ...
    private fun notifyCallbacksForNotificationClick(
        actionType: SendsayNotificationActionType,
        actionUrl: String?,
