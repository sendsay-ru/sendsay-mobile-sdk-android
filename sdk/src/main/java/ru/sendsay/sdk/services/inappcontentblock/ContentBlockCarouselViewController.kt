...
import ru.sendsay.sdk.models.InAppContentBlockCallback
...
internal class ContentBlockCarouselViewController(
    ...
    fun reload() {
        ...
        runForInitializedSDK(RELOAD_PROCESS_ID) {
            ensureOnBackgroundThread {
                val manager = Sendsay.getComponent()?.inAppContentBlockManager
                if (manager == null) {
                    Logger.e(this, "InAppCbCarousel: Sendsay SDK is not initialized properly")
    ...
    private fun filterContentBlocks(source: List<InAppContentBlock>): List<InAppContentBlock> {
        val manager = Sendsay.getComponent()?.inAppContentBlockManager
        if (manager == null) {
            Logger.e(this, "InAppCbCarousel: Sendsay SDK is not initialized, unable to filter content blocks")
            return source
        }
    ...
    fun onViewAttachedToWindow() {
        ...
        SendsayContextProvider.registerForegroundStateListener(this)
        reload()
    }
    ...
    private fun shouldBeRemovedAfterAction(contentBlock: InAppContentBlock): Boolean {
        ...
        if (manager == null) {
            Logger.e(
                this,
                "InAppCbCarousel: Sendsay SDK is not initialized properly, unable to check removal by action"
            )
            return false
        }
