...
import ru.sendsay.example.databinding.FragmentInappContentBlocksBinding
import ru.sendsay.example.models.Constants
import ru.sendsay.example.view.MainActivity
...
//    }
    ...
    private fun prepareExampleAndroidCbPlaceholder() {
        ...
        Sendsay.getInAppContentBlocksPlaceholder("ph_x_example_Android", requireContext())?.let {
...
val examplePlaceholderHeightInfoView = TextView(requireContext()).apply { text = "Height: 0px" }
        Sendsay.getInAppContentBlocksPlaceholder(
            "example_top",
            requireContext(),
            ...
            it.behaviourCallback = object : InAppContentBlockCallback {
                ...
                override fun onCloseClicked(placeholderId: String, contentBlock: InAppContentBlock) {
                    Sendsay.trackInAppContentBlockCloseWithoutTrackingConsent(placeholderId, contentBlock)
                }
...
class ProductsAdapter(private val data: List<ProductsViewModel>) : Adapter<ProductsAdapter.ViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        ...
        return when (viewType) {
            CONTENT_BLOCK_TYPE -> {
                var contentBlocksPlaceholder: View? = Sendsay.getInAppContentBlocksPlaceholder(
                    "example_list",
                    parent.context,
                ...
                if (contentBlocksPlaceholder == null) {
                    // Sendsay SDK is not initialized
